...
app.post('/food', Middleware.auth, Middleware.validNewFood, Middleware.error_handling, function (req: any, res: any, next:any) {
  FoodController.createFood(req, res, next);
});

// 2) PUT /food/ <id> -> modifica alimento <id> + name, quantity (opzionali)
app.put('/food/:id', Middleware.auth, Middleware.validUpdFood, Middleware.error_handling, function (req: any, res: any, next:any) {
...
app.post('/order', Middleware.auth, Middleware.validOrder, Middleware.error_handling, function (req: any, res: any, next:any) {    
  OrderController.createOrder(req, res, next);
});

// 4) POST /order/<id_order> -> Presa in carico ordine <id_order>
app.post('/order/:id', Middleware.auth, Middleware.takeOrder, Middleware.error_handling, function (req: any, res: any, next:any) {
  OrderController.takeOrder(req, res, next);
});

// 5) GET /order/<id_order> -> dati ordine <id_order>
app.get('/order/:id', Middleware.auth, Middleware.orderState,Middleware.error_handling, function (req: any, res: any, next:any) {
...
app.post('/load/:id', Middleware.auth, Middleware.addLoad, Middleware.error_handling, function (req: any, res: any, next:any) {    
  OrderController.addLoad(req, res, next);
});

// 8) POST /admin/token -> re-impostare il numero token di un utente + user_id, token_amount
app.post('/admin/token', Middleware.isAdmin, Middleware.updToken, Middleware.error_handling, function (req: any, res: any, next:any) {    
